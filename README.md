...
* **Framework:** Flutter 3.27.1+ / Dart 3 (проверки закреплены на Flutter 3.27.1)
...
## UI-превью и проверки

Главный экран поддерживает локальный поиск по имени/нику и фильтры
«Все», «Непрочитанные», «Группы». Поиск в списке не обращается к серверу.
Фоновые обновления сохраняют видимый список; при ошибке можно повторить запрос.

В Hoplite настройка и запуск описаны в `.hoplite/settings.json`:
`setup.sh` устанавливает Flutter 3.27.1 с проверкой SHA-256 в игнорируемый
`.hoplite/cache/`, загружает зависимости и генерирует модели. `run.sh` собирает
отдельную точку входа `tool/preview.dart` и раздаёт её на порту 3000.

Это **изолированное UI-превью**, а не клиент production Supabase: используются
настоящие экраны и контроллеры, но репозитории работают на вымышленных данных
в памяти. Можно проверить поиск, фильтры, светлую/тёмную тему и текстовую
переписку. Вход, профиль, создание групп, вложения и push-уведомления не
подключены. Превью не требует ключей и не меняет реальные переписки.
Обычная мобильная сборка по-прежнему использует `lib/main.dart`.

```bash
bash .hoplite/setup.sh
bash tool/check.sh       # flutter analyze, flutter test, git diff --check
```

Новые regression-тесты проверяют гонки обновления и поиска, фильтры,
приватность кэша, черновики, ошибки авторизации и адаптивность UI.
`.github/workflows/flutter-checks.yml` выполняет анализ, тесты и сборку
UI-превью на каждом pull request. Это не заменяет тестирование Android/iOS
на устройстве с отдельным тестовым проектом Supabase.
...
Локальная схема v9 изолирует профили, диалоги, сообщения и реакции по
`account_id`. При обновлении старый кэш без владельца очищается и заново
загружается из Supabase; серверные данные не удаляются. Для первого открытия
после обновления потребуется подключение к сети.

Новые черновики также привязаны к аккаунту. Старые черновики без информации
о владельце не подставляются автоматически: безопасно определить их аккаунт
невозможно. TLS-сертификаты проверяются системным хранилищем доверия,
в том числе при использовании прокси.
